...
export class XrayGeneratorService {
        ...
        switch (host.protocol) {
            ...
            case 'shadowsocks':
                return this.shadowsocks({
                    ...
                    port: host.port,
                    method: host.encryption || '2022-blake3-aes-256-gcm',
    ...
    private socks(params: IFormattedHost): string | undefined {
    const credentials = params.socksCredentials;
    const scheme = 'socks5';

    let authSegment = '';
    const requiresAuth = credentials?.auth === undefined || credentials?.auth === 'password';
    ...
    if (credentials && requiresAuth) {
        ...
        if (username.length || password.length) {
            authSegment = `${encodeURIComponent(username)}:${encodeURIComponent(password)}@`;
        }
    }

    const authority = `${authSegment}${params.address}:${params.port}`;
    const queryParams: string[] = [];

    if (credentials?.udp === true) {
        queryParams.push('udp=true');
    }

    const queryString = queryParams.length ? `?${queryParams.join('&')}` : '';

    return `${scheme}://${authority}${queryString}#${encodeURIComponent(params.remark)}`;
}
